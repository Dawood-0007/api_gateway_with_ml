...
const ApiPlayground = ({ initialData, contain }: Props) => {

    const { data: timeData, mutate: mutateTimeData } = useSWR(
        "http://localhost:5000/api/stat/hourlyData",
        fetcher,
        { fallbackData: initialData }
    );
    
    const { data: ipStatus, mutate: mutateIpStatus } = useSWR(
        "http://localhost:5000/api/stat/checkIp",
        fetcher,
        { fallbackData: contain }
    );

    const [logs, setLogs] = useState<LogEntry[]>([]);
    const [loading, setLoading] = useState<string | null>(null);
    const logRef = useRef<HTMLDivElement>(null);
    const [isBlocked, setIsBlocked] = useState<boolean>(ipStatus ?? false);
    ...
    const simulate = useCallback(async (type: string) => {
        ...
        await mutateTimeData();
        await mutateIpStatus();
