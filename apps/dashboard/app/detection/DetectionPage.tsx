...
type DetectionPageData = {
  requests: Logs[];
  distribution: DetectionPage;
};

type Props = {
  initialData: DetectionPageData;
};


const ThreatDetection = ({ initialData }: Props) => {
  ...
  const [requests, setRequests] = useState<DetectionPage>(
  ...
  const anomalyDistribution = [
    { range: '0.0-0.2', count: requests["0-0.2"] },
    { range: '0.2-0.4', count: requests["0.3-0.4"] },
    { range: '0.4-0.6', count: requests["0.5-0.6"] },
    { range: '0.6-0.8', count: requests["0.7-0.8"] },
    { range: '0.8-2.0', count: requests["0.9-1.0"] }
  ];
